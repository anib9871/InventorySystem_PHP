...
/*--------------------------------------------------------------*/
/* Function for Send Invoice PDF via Brevo API
/*--------------------------------------------------------------*/
function send_invoice_email($invoice_id, $to_email, $customer_name) {
    global $db;

    // 1. Dompdf Autoload Include
    ...
    if (file_exists($vendor_file)) {
        ...
        error_log("Dompdf Autoload file not found at: " . $vendor_file);
        return false;
    }
    ...
    // 4. Build Professional Invoice HTML matching print exact UI
    ...
    // 6. Brevo Configuration
    $apiKey = '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';
    $senderEmail = 'redorangesconsulting@gmail.com';

    $data = [
        "sender" => ["name" => "Inventory System", "email" => $senderEmail],
        "to" => [["email" => $to_email, "name" => $customer_name]],
        "subject" => "Invoice #" . $invoice['invoice_no'] . " from Inventory System",
        "htmlContent" => "Dear <b>" . htmlspecialchars($customer_name) . "</b>,<br><br>Please find attached your invoice PDF.<br><br>Regards,<br><b>Team Inventory</b>",
        "attachment" => [
            [
                "content" => $pdf_base64,
