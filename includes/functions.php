...
/*--------------------------------------------------------------*/
/* SMART DOMPDF MANUAL INITIALIZER
/*--------------------------------------------------------------*/
function load_dompdf_framework() {
    ...
    if ($v) {
        ...
        }, true, true); // High priority load

        // Load CPDF Adapter directly
        $cpdf_file = $v . '/dompdf/dompdf/src/Adapter/CPDF.php';
...
function send_invoice_email($invoice_id, $to_email, $customer_name) {
    ...
    if (!load_dompdf_framework()) {
        return "Dompdf load error: Vendor paths not found.";
    }
