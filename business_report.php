...
/* ═══════════════════════════════════════
   1. TOTAL SALE
═══════════════════════════════════════ */
$sale_query = "SELECT SUM(sale_net) as total_sale FROM transaction_master
               WHERE transaction_type = 2 AND DATE(entry_date) BETWEEN '{$from}' AND '{$to}'";
...
/* ═══════════════════════════════════════
   4. TRANSACTION LIST
═══════════════════════════════════════ */
$txn_q = "SELECT t.transaction_id, t.bill_indent_no AS invoice_no, t.entry_date,
            p.name, t.quantity, t.unit_price, t.sale_gst, t.sale_net, mc.center_name
...
if ($role_id == 3)                               $txn_q .= " AND t.center_id = '{$user_center}'";
elseif ($role_id == 2 && !empty($center_filter)) $txn_q .= " AND t.center_id = '{$center_filter}'";
$txn_q .= " ORDER BY mc.center_name ASC, t.transaction_id DESC";
...
<div class="pdf-total-box" style="display:none;">
  ...
  <div class="rpt-top" style="height:185px;">
    ...
    <!-- 3. Center Pie + Panel (admin only) -->
    <?php if ($role_id == 2): ?>
    <div class="rpt-card rpt-center">
      <div class="rpt-card-title">Center Wise Sales</div>
      <div class="rpt-pie-row" style="height:145px;">
        <div class="rpt-pie-box">
          <canvas id="centerChart" style="width:100%; height:100%;"></canvas>
        </div>
        <div class="rpt-pie-panel">
          <?php foreach ($center_sales as $ci => $cs):
            $col = $pie_colors[$ci % count($pie_colors)];
            $modeRows = [];
            if (!empty($cs['payment_modes']))
              foreach (explode(' | ', $cs['payment_modes']) as $part)
                $modeRows[] = array_map('trim', explode(' : ', trim($part)));
          ?>
          <div class="rpt-ci" style="border-color:<?= $col ?>;">
            <div class="rpt-ci-name"><?= htmlspecialchars($cs['center_name']) ?></div>
            <div class="rpt-ci-total" style="color:<?= $col ?>;">&#8377;<?= number_format($cs['total_sale'], 2) ?></div>
            <?php foreach ($modeRows as $mr): ?>
            <div class="rpt-ci-mode">
              <span><?= htmlspecialchars($mr[0] ?? '') ?></span>
              <span><?= htmlspecialchars($mr[1] ?? '') ?></span>
            </div>
            <?php endforeach; ?>
          </div>
          <?php endforeach; ?>
...
if(!empty($center_filter)){
  ...
  echo ($role_id == 2) ? 'CENTER WISE SALES DETAIL' : 'SALES DETAIL';
}
?>
...
foreach ($sales as $index => $s):
    ...
    // NEW CENTER START
if ($current_center != $s['center_name']) {

    // PREVIOUS CENTER TOTAL
    if ($current_center != '') {
?>

<tr style="background:#e2e8f0;font-weight:700;">
    <td colspan="<?= ($gst_enabled == 'Yes') ? '6' : '5' ?>" style="text-align:right;">
        Total of <?= htmlspecialchars($current_center) ?>
    </td>
    <td>
        <b>₹ <?= number_format($center_total,2) ?></b>
    </td>
</tr>

<tr>
    <td colspan="<?= ($gst_enabled == 'Yes') ? '7' : '6' ?>"
        style="
          height:22px;
          border:none !important;
...
<?php
    ...
    // RESET TOTAL
    $center_total = 0;
    $current_center = $s['center_name'];
?>

<!-- CENTER HEADING -->
<tr>
<td colspan="<?= ($gst_enabled == 'Yes') ? '7' : '6' ?>" class="center-group-heading">
        <?= htmlspecialchars($current_center) ?> SALES DETAIL
    </td>
</tr>
...
<?php
    ...
    $center_total += $s['sale_net'];
?>

<tr>
    <td><?= date('d-M-Y', strtotime($s['entry_date'])) ?></td>
...
<?php if($gst_enabled == "Yes"): ?>
<td style="text-align:center;">
    <?= number_format($s['sale_gst'],2) ?>%
</td>
<?php endif; ?>
    <td>
        <b>₹ <?= number_format($s['sale_net'],2) ?></b>
    </td>
</tr>
