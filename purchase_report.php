...
// Product filter ke liye Ledger table ka use kiya gaya hai
if($report_type=='product' && !empty($filter_id)){
    $pay_q .= " AND sp.ledger_id IN (
        SELECT ledger_id FROM supplier_ledger 
        WHERE bill_no IN (
            SELECT bill_indent_no FROM transaction_master 
            WHERE product_id='{$filter_id}' AND transaction_type=1
        )
    )";
}

if ($role_id == 3)                               $pay_q .= " AND sp.center_id = '{$user_center}'";
elseif ($role_id == 2 && !empty($center_filter)) $pay_q .= " AND sp.center_id = '{$center_filter}'";
...
if(!empty($payments)) {
    foreach ($payments as $pay){
        ...
        if(!isset($mode_summary[$mode])) {
            $mode_summary[$mode] = 0;
        }
        $mode_summary[$mode] += (float)$pay['payment_amount'];
    }
}
...
if ($role_id == 2) {
    $cq = "SELECT x.center_name, x.center_id, SUM(x.mode_total) as total_sale,
           ...
           FROM (
             SELECT mc.center_name, sp.center_id, sp.payment_mode, SUM(sp.payment_amount) as mode_total
             FROM supplier_payment sp
             LEFT JOIN master_center mc ON mc.center_id = sp.center_id
             WHERE DATE(sp.payment_date) BETWEEN '{$from}' AND '{$to}'";
    if (!empty($center_filter)) $cq .= " AND sp.center_id = '{$center_filter}'";
    $cq .= " GROUP BY sp.center_id, sp.payment_mode) x GROUP BY x.center_id, x.center_name";
...
SELECT
    ...
    -- Subquery using supplier_ledger logic you provided
    (SELECT IFNULL(SUM(sp_sub.payment_amount), 0) 
     FROM supplier_payment sp_sub 
     LEFT JOIN supplier_ledger sl_sub ON sp_sub.ledger_id = sl_sub.ledger_id 
     WHERE sl_sub.bill_no = t.bill_indent_no) AS paid_amount
...
SUM(
    ...
    CASE
        ...
        THEN IFNULL((SELECT SUM(s.total_amount) FROM shipping s WHERE s.bill_no = t.bill_indent_no),0)
        ELSE 0
    END
) AS price
FROM transaction_master t
LEFT JOIN products p ON p.id = t.product_id
WHERE t.transaction_type = 1 AND t.supplier_id IS NOT NULL AND t.supplier_id != 0 AND p.type = 1
AND DATE(t.entry_date) BETWEEN '{$from}' AND '{$to}'
";
...
FROM transaction_master t
LEFT JOIN supplier_master sm ON sm.id = t.supplier_id
WHERE t.transaction_type = 1 AND t.supplier_id IS NOT NULL AND t.supplier_id != 0
AND DATE(t.entry_date) BETWEEN '{$from}' AND '{$to}'
";
...
.rpt-header h2 { font-size: 16px; font-weight: 800; text-transform: uppercase; letter-spacing: .07em; color: #0f172a; margin: 0 0 2px; }
...
@media screen and (max-width: 768px) {
  ...
  .mobile-wrap-form > input, .mobile-wrap-form > select { flex: 1 1 calc(50% - 6px) !important; min-width: 130px; }
...
@media print {
  ...
  * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
  ...
  .pdf-period-box { display:block !important; background:#b30000 !important; color:#fff !important; padding:5px 9px !important; border-radius:3px !important; margin-bottom:6px !important; font-size:13px !important; font-weight:700 !important; line-height:1.2 !important; }
  .pdf-total-box { display:block !important; background:#0f172a !important; color:#fff !important; padding:5px 9px !important; border-radius:3px !important; margin-bottom:6px !important; font-size:13px !important; font-weight:700 !important; line-height:1.2 !important; }
  ...
  .pdf-collection-box { display:block !important; background:linear-gradient(90deg,#a10805,#111827) !important; color:#fff !important; padding:6px 10px !important; border-radius:4px !important; margin-bottom:8px !important; width:100% !important; font-size:12px !important; line-height:1.2 !important; }
...
<div class="rpt-header">
  ...
  <h3 style="margin:4px 0 6px; font-size:15px; font-weight:700; color:#334155; letter-spacing:.04em; text-transform:uppercase;">Purchase Report</h3>
  ...
  <div class="rpt-filter no-print">
    <form method="post" class="mobile-wrap-form" style="display:flex; align-items:center; gap:6px; flex-wrap:nowrap; width:100%; margin:0;">
...
<div class="pdf-total-box" style="display:none;">
  ...
  <div class="pdf-collection-box" style="display:none;">
    <h4 style="margin:0 0 8px; font-size:16px; font-weight:700; color:#fff;">Supplier Payment Summary (Mode-wise)</h4>
    ...
    <tbody>
        ...
        <tr>
            ...
            <?php 
                ...
                // Pending check (Blue if paid, Red if pending)
