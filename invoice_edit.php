...
/* 🔥 DEMO ITEM POP-UP & IMPORT LOGIC (EDIT MODE FIXED) 🔥 */
let currentDemoData = [];

// 1. Core Function: Demo Items Fetch Karke Modal Dikhaney Ke Liye
    ...
    fetch(`get_demo_items.php?customer_id=${custId}`)
        ...
        .then(data => {
            if (data && data.length > 0) {
                ...
                // Bootstrap Modal Pop-up Display (Ensures it shows on screen)
...
// 2. Initial Page Load Check (Jaise hi Edit Page kholein, tabhi pop-up aayega agar demo item baki hai)
document.addEventListener("DOMContentLoaded", function () {
    let initialCustId = document.querySelector("select[name='customer_id']").value;
    if (initialCustId) {
        // 500ms delay taaki page ka DOM aur Bootstrap completely render ho jaye
        setTimeout(() => {
            checkAndShowDemoModal(initialCustId);
        }, 500);
    }
});

// 3. Dropdown Change Listener (Jab customer change kiya jaye)
...
// Import Selected Items to Invoice Grid
