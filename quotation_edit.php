...
$items = find_by_sql("
SELECT qi.*, p.name, p.sale_price, p.type
...
/* ================= TAX MODE ================= */
$org_id = $_SESSION['org_id'] ?? $quote['organization_id'];
...
<form method="post" onsubmit="return validateQuotation()">
  ...
  <div class="pos-card" style="padding: 8px 14px;">
    <div class="row align-items-end">
      ...
      <div class="col-xs-12 col-sm-6 col-md-3" style="margin-bottom: 4px;">
        ...
        <input type="text" name="quotation_date" id="quotation_date" class="form-control" value="<?= date('d/M/Y', strtotime($quote['quotation_date'])); ?>" autocomplete="off">
      </div>
  ...
  <!-- FULL-WIDTH BILLING DATA GRID (SWEET SPOT: 220px) -->
        ...
        <tbody id="billBody">
          ...
          <tr id="emptyRowMsg">
            <td colspan="<?= ($gst_enabled == 'Yes') ? 9 : 7; ?>" class="empty-grid-msg">
              No products in this quotation. Click <b>"Add Products"</b> above to insert items.
            </td>
          </tr>
          <?php else: ?>
    ...
    <div class="row">
      ...
      <div class="col-xs-12 col-md-6" style="margin-bottom: 6px;">
        <div class="summary-metric-card">
          ...
          <div class="stat-row">
            ...
            <strong>₹ <span id="gstTotal"><?= number_format((float)$quote['gst_total'], 2); ?></span></strong>
          </div>
      ...
      <div class="col-xs-12 col-md-6" style="display:flex; flex-direction:column; justify-content:space-between;">
        <div>
            ...
            <div class="col-xs-8">
              <select id="termsTemplate" class="form-control" style="font-size:12px; height:30px !important;">
                ...
                <?php foreach($terms_templates as $t): ?>
                  <option value="<?= htmlspecialchars($t['template'] ?? ''); ?>" <?= trim((string)($quote['terms_conditions'] ?? '')) == trim((string)($t['template'] ?? '')) ? 'selected' : ''; ?>>
          ...
          <textarea name="terms_conditions" id="termsBox" class="form-control" style="height: 75px !important; resize: none; font-size:12px; margin-bottom: 6px;" placeholder="Add quotation terms, validity or remarks..."><?= htmlspecialchars($quote['terms_conditions'] ?? ''); ?></textarea>
        </div>
        
        <button type="submit" name="update_quotation" class="btn btn-save-master">
