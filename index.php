...
<style>
  ...
  /* 🏬 SUPER PREMIUM 3D METALLIC SHUTTER OVERLAY FOR LOGIN */
  .shutter-overlay {
      position: fixed;
      top: 0;
      left: 0;
  ...
  .shutter-overlay.shutter-active {
      transform: translateY(0%) !important;
      pointer-events: all;
  }

  /* 3D Character & Badge Overlay */
...
<div id="loginShutter" class="shutter-overlay">
    <div class="character-3d-wrapper">
        <div id="shutterStatusBadge" class="character-3d-badge">🔒 VERIFYING & CLOSING STORE...</div>
        
        <!-- Modern 3D Business Avatar SVG -->
        ...
        <div class="shutter-handle-bar">
            <i class="fa-solid fa-store" style="margin-right: 8px; color: #d97706;"></i> STORE GATE
        </div>
    </div>
...
<!-- ✅ SOUND SYNTHESIZER & ANIMATION SYNC CONTROLLER -->
<script>
// Web Audio API to play metallic sounds smoothly without loading delay
...
window.addEventListener("DOMContentLoaded", function() {
    ...
    if (form) {
        form.addEventListener("submit", function(e) {
            // HTML5 Form Validation check (Username/Password empty to nahi hai)
            if (!form.checkValidity()) {
                return; // Normal browser alert dikhane do
            }
            ...
            if (shutter) {
                // 1. Rolling Sound & Animation start saath me
                playRollingShutterSound();
                shutter.classList.add("shutter-active");

                // 2. Lock sound jab shutter complete niche aa jaye (~850ms)
                setTimeout(function() {
                    playLockClickSound();
                }, 850);

                // 3. Form submit jab saari sound aur animation khtm ho jaye (~1100ms)
                setTimeout(function() {
                    form.submit();
                }, 1100);
            } else {
